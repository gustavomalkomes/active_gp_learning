classdef CovarianceNumericalTest < matlab.unittest.TestCase
    methods (Test)
        function test_hessians(testCase)
            %% parameters
            ...
            [diff,a,b] = check_hessian(fun_fx,fun_dx,randn(n_param,1),g_tol,n);
            testCase.assertTrue(diff<e_tol,'isotropic_rq_covariance Hessian doesnt match');
            ...
            [diff,a,b] = check_hessian(fun_fx,fun_dx,randn(n_param,1),g_tol,n);
            testCase.assertTrue(diff<e_tol,'sum_covariance Hessian doesnt match');
            ...
            [diff,a,b] = check_hessian(fun_fx,fun_dx,randn(n_param,1),g_tol,n);
            testCase.assertTrue(diff<e_tol,'prod_covariance Hessian doesnt match');
            ...
            [diff,a,b] = check_hessian(fun_fx,fun_dx,randn(n_param,1),g_tol,n);
            testCase.assertTrue(diff<e_tol,'periodic_covariance Hessian doesnt match');
            ...
            testCase.assertTrue(diff<e_tol,'isotropic_sqdexp_covariance Hessian doesnt match');
            
            %% checking isotropic_matern_covariance 1/2
            n_param = 2;
            fun_fx = @(hyp,i)   isotropic_matern_covariance12(hyp,xx,zz,i);
            fun_dx = @(hyp,i,j) isotropic_matern_covariance12(hyp,xx,zz,i,j);
            
            diff = check_hessian(fun_fx,fun_dx,randn(n_param,1),g_tol,n);
            testCase.assertTrue(diff<e_tol,'isotropic_matern_covariance12 Hessian doesnt match');
        end
    end
end